  ...
  void CalcDesiredState(const Context<double>& context,
    ...
    if (context.get_time() > lift_start_) {
      ...
      if ((context.get_time() >= shake_start_ + delta_t_) && (context.get_time() < shake_start_ + 3 * delta_t_)) {
        fraction = 1.0 - (context.get_time() - (shake_start_ + delta_t_)) / delta_t_;
        target_z_pos -= fraction * 0.18 * box_width_;
      }
      if ((context.get_time() >= shake_start_ + 3 * delta_t_) && (context.get_time() < shake_start_ + 5 * delta_t_)) {
        fraction = 1.0 - (context.get_time() - (shake_start_ + 3 * delta_t_)) / delta_t_;
        target_z_pos += fraction * 0.18 * box_width_;
      }
...
int do_main() {
  ...
  AddContactMaterial(FLAGS_damping, {}, surface_friction_ground, &rigid_hydro_props);
