...
class SparseGrid {
 public:
  ...
  void Reserve(size_t capacity) {
    ...
    // the above three attributes all have length num_active_nodes(), which is
    // dynamic batch_indices_ has a fixed size of Particles.num_particles()
    batch_indices_.reserve(capacity);
  }

  /// Given a list of particle positions, first identifies and marks the active
  /// grid nods, then sorts them lexicographically.
  /// @note User should call this function to update the SparseGrid whenever the
  /// underlying particle positions changes, by *only* calling
  /// this.Update(particles.positions()). All other functionalities of this
  /// class will then be up-to-date.
  ...
  const std::vector<Vector3<int>>& batch_indices() const { return batch_indices_; }

  /// Given particle p, returns its 3d batch index.
  const Vector3<int>& GetParticleBatchIndexAt(size_t p) const {
 ...
 private:
  ...
  /// {i, j, k} is called the batch index of this particle. The information is
  /// stored in batch_indices_
  /// @note batch_indices_.size() = positions.size()
