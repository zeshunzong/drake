...
class AnalyticLevelSet {
 public:
  ...
  /* Returns the outward unit normal of an interior point at position in the
   analytic level set.
   @throws std::exception if the point is in exterior. */
  ...
  /* Returns the bounding box of the geometry.
    We denote the first component of bounding_box_ as xmin_, second component is
    xmax_. xmin_, xmax_ represents the bounding box of the geometry. i.e. the
    geometry lies in [xmin_(0), xmax_(0)]X[xmin_(1), xmax_(1)]X[xmin_(2),
    xmax_(2)]. */
...
// An analytic level set class for sphere centered at (0,0,0) with given radius
class SphereLevelSet : public AnalyticLevelSet {
 public:
  ...
  /*
   ...
   For the singularity x = (0, 0, 0), we defines its outward normal vector to be
   (1, 0, 0).
   */
  Vector3<double> DoGetNormal(const Vector3<double>& position) const final;
...
class CylinderLevelSet : public AnalyticLevelSet {
 public:
  ...
  /* The outward normal for a point on the cylindrical surface (including its
   ...
   outward normt al for a point on the top/bottom disk (excluding boundary) is
   (0,0,1)/(0,0,-1) The outward normal for an interior point is the normal
   of its closest surface. When the point is equidistant from the (either top
   or bottom) disk and the cylindrical surface, the normal is taken to be that
   of the cylindrical surface (N.B. there is a discontinuity here). */
  Vector3<double> DoGetNormal(const Vector3<double>& position) const final;
