...
MPMDriver::MPMDriver(MPMParameters param):
                                ...
                                grid_(Vector3<int> {0,0,0}, param.solver_param.h, Vector3<int> {2,3,4}),
...
void MPMDriver::WriteParticlesToBgeo(int io_step) {
    ...
    std::filesystem::create_directories(param_.io_param.output_directory);
    internal::WriteParticlesToBgeo(output_filename, particles_.get_positions(),
                                                    particles_.get_velocities(),
                                                    particles_.get_masses());
    std::string output_filename_obj = param_.io_param.output_directory + "/"
                                + param_.io_param.case_name
                                + std::to_string(io_step) + ".obj";                                                
    internal::WriteParticlesToObj(output_filename_obj, particles_.get_positions());
}
