...
class MpmState {
 public:
  ...
  MpmState(const Particles<T>& particles){
    particles_ = particles;
  }

  const Particles<T>& GetParticles() const {
    return particles_;
  }

  void SetParticles(const Particles<T>& particles) {
    particles_ = particles;
  }

  void SetState(const MpmState<T>& mpmstate){
