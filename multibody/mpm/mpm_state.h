...
namespace mpm {
// TO BE MODIFIED
/** %MpmState provides access to private workspace FEM state and per-element
...
class MpmState {
 public:
  ...
  /** Returns the number of degrees of freedom in the MPM model and state. */
  int num_dofs() const {
    return get_context()
  ...
  bool is_created_from_system(const internal::MpmStateSystem<T>& system) const {
 ...
 private:
  ...
  const internal::MpmStateSystem<T>* system_{nullptr};
