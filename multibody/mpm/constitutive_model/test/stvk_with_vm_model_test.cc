...
GTEST_TEST(StvkWithVonMisesModelTest, TestReturnMapAndStress) {
    double E0      = 5.0;
    double nu0     = 0.25;
    double tau_c0  = 1e6;
    double E1      = 23.4;
...
GTEST_TEST(StvkWithVonMisesModelTest, TestEnergyDerivatives) {
  ...
  StvkHenckyWithVonMisesModel<AutoDiffXd> model_old(3.14159, 0.3337, 1e6);
  StvkHenckyWithVonMisesModel2<AutoDiffXd> model(3.14159, 0.3337, 1e6);
  Matrix3<AutoDiffXd> F = MakeDeformationGradientsWithDerivatives();
  AutoDiffXd f_old = model_old.CalcStrainEnergyDensity(F);
  AutoDiffXd f = model.CalcStrainEnergyDensity(F);
  EXPECT_EQ(f_old, f);
  Matrix3<AutoDiffXd> dfdF;
  model.CalcFirstPiolaStress(F, &dfdF);
  Matrix3<AutoDiffXd> dfdF_old;
  model_old.CalcFirstPiolaStress(F, &dfdF_old);
  EXPECT_EQ(dfdF, dfdF_old);
  EXPECT_TRUE(
