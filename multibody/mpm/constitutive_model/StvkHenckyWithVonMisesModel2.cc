...
template <typename T>
StvkHenckyWithVonMisesModel2<T>::StvkHenckyWithVonMisesModel2(const T& E, const T& nu,
                                                         const T& tau_c):
...
template <typename T>
StvkHenckyWithVonMisesModel2<T>::StvkHenckyWithVonMisesModel2(const T& tau_c):
                                    ElastoPlasticModel<T>(), yield_stress_(tau_c) {
    DRAKE_ASSERT(tau_c > 0);
}

template <typename T>
T StvkHenckyWithVonMisesModel2<T>::CalcStrainEnergyDensity(const Matrix3<T>& FE) const {  
...
template <typename T>
T StvkHenckyWithVonMisesModel2<T>::EvalYieldFunction(const Matrix3<T>& FE)
                                                                        const {
...
template <typename T>
void StvkHenckyWithVonMisesModel2<T>::CalcFEFromFtrial(Matrix3<T>* F_trial) const {
      DRAKE_ASSERT(F_trial != nullptr);
      const StrainStressData strain_stress_data_trial = ComputeStrainStressData(*F_trial);
      ApplyReturnMapping(strain_stress_data_trial, F_trial);
}


template <typename T>
void StvkHenckyWithVonMisesModel2<T>::CalcFirstPiolaStress(const Matrix3<T>& FE, Matrix3<T>* P) const {
    // P = U (2 mu S^{-1} (log S) + lambda tr(log S) S^{-1}) V^T
    // StrainData strain_data = CalcStrainData(FE);

    const StrainStressData strain_stress_data = ComputeStrainStressData(FE);
    Vector3<T> P_hat = 2 * this->mu_ * strain_stress_data.log_Sigma + this->lambda_ * strain_stress_data.log_Sigma_trace*Vector3<T>::Ones();
    ...
    (*P) = strain_stress_data.U * P_hat.asDiagonal() * strain_stress_data.V.transpose();
}

template <typename T>
void StvkHenckyWithVonMisesModel2<T>::CalcKirchhoffStress(const Matrix3<T>& FE, Matrix3<T>* tau) const {
    ...
    Vector3<T> sigma_tau;
    for (int d = 0; d < 3; ++d) {
        sigma_tau(d)  = 2*ElastoPlasticModel<T>::mu_*strain_data.eps_hat(d)
                      + ElastoPlasticModel<T>::lambda_*strain_data.tr_eps;
        sigma_tau(d) = 2*this->mu_*strain_stress_data.log_Sigma(d) + this->lambda_*strain_stress_data.log_Sigma_trace;

    }
    *tau = strain_stress_data.U * sigma_tau.asDiagonal() * strain_stress_data.U.transpose();
    // The Kirchhoff stress can be then written as
    // τ = U Σᵢ Uᵀ, where U are left singular vectors of the elastic deformation
    //             gradient FE
    // *tau = strain_data.U * sigma_tau.asDiagonal() * strain_data.U.transpose();

}

template <typename T>
void StvkHenckyWithVonMisesModel2<T>::CalcFirstPiolaStressDerivative(const Matrix3<T>& FE, Eigen::Matrix<T, 9, 9>* dPdF) const {
    dPdF->setZero();
...
template <typename T>
void StvkHenckyWithVonMisesModel2<T>::
    ...
    CalcKirchhoffStressPrivate(trial_strain_data, tau);
}
...
template <typename T>
StvkHenckyWithVonMisesModel2<T>::StrainData
        StvkHenckyWithVonMisesModel2<T>::CalcStrainData(const Matrix3<T>& FE)
                                                                        const {
    Eigen::JacobiSVD<Matrix3<T>>
                    svd(FE, Eigen::ComputeFullU | Eigen::ComputeFullV);
...
StvkHenckyWithVonMisesModel2<T>::StrainStressData 
        StvkHenckyWithVonMisesModel2<T>::ComputeStrainStressData(const Matrix3<T>& F_trial) const {
        Eigen::JacobiSVD<Matrix3<T>>
                    svd(F_trial, Eigen::ComputeFullU | Eigen::ComputeFullV);
    const Matrix3<T>& U = svd.matrixU();
    const Matrix3<T>& V = svd.matrixV();
    Vector3<T> Sigma    = svd.singularValues();
    ...
    // In the principal frame: dev(τ) = 2μlog(σᵢ) - 2μ/3 ∑ᵢ log(σᵢ) = 2μ[log(σᵢ)-1/3*∑ᵢ log(σᵢ)]
    Vector3<T> deviatoric_tau = 2*this->mu_*(log_Sigma-1.0/3.0*log_Sigma_trace*Vector3<T>::Ones());
    return {U, V, Sigma, OneOverSigma, log_Sigma, log_Sigma_trace, deviatoric_tau};
}


template <typename T>
StvkHenckyWithVonMisesModel2<T>::StressData
        StvkHenckyWithVonMisesModel2<T>::CalcStressData(
                const StvkHenckyWithVonMisesModel2<T>::StrainData& strain_data)
                                                                        const {
...
template <typename T>
StvkHenckyWithVonMisesModel2<T>::PsiSigmaDerivative 
        StvkHenckyWithVonMisesModel2<T>::CalcPsiSigmaDerivative(const Matrix3<T>& FE) const {
    StrainData strain_data = CalcStrainData(FE);
    ...
    T m01 = -(this->lambda_ * strain_data.tr_eps + 
                                    this->mu_ * 2 * mpm::internal::CalcXLogYMinusYLogXOverXMinusY<T>(strain_data.sigma(0), abs(strain_data.sigma(1)))) / (strain_data.sigma(0) * strain_data.sigma(1));
    T m02 = -(this->lambda_ * strain_data.tr_eps + 
                                    this->mu_ * 2 * mpm::internal::CalcXLogYMinusYLogXOverXMinusY<T>(strain_data.sigma(0), abs(strain_data.sigma(2)))) / (strain_data.sigma(0) * strain_data.sigma(2));
    T m12 = -(this->lambda_ * strain_data.tr_eps + 
                                    this->mu_ * 2 * mpm::internal::CalcXLogYMinusYLogXOverXMinusY<T>(strain_data.sigma(1), abs(strain_data.sigma(2)))) / (strain_data.sigma(1) * strain_data.sigma(2));
    // (psiA+psiB)/(sigmaA+sigmaB)
    T p01 = (psi0 + psi1) / mpm::internal::ClampToEpsilon<T>(strain_data.sigma(0) + strain_data.sigma(1), epsilon_threshold_);
    T p02 = (psi0 + psi2) / mpm::internal::ClampToEpsilon<T>(strain_data.sigma(0) + strain_data.sigma(2), epsilon_threshold_);
    T p12 = (psi1 + psi2) / mpm::internal::ClampToEpsilon<T>(strain_data.sigma(1) + strain_data.sigma(2), epsilon_threshold_);
...
template <typename T>
T StvkHenckyWithVonMisesModel2<T>::EvalYieldFunction(const StressData&
                                                      trial_stress_data) const {
    return StvkHenckyWithVonMisesModel2<T>::sqrt_32
          *trial_stress_data.tau_dev_hat_norm - yield_stress_;
}

template <typename T>
T StvkHenckyWithVonMisesModel2<T>::EvalYieldFunction(const Vector3<T>& deviatoric_tau) const {
    return kSqrt3Over2*deviatoric_tau.norm() - yield_stress_;
}

template <typename T>
void StvkHenckyWithVonMisesModel2<T>::CalcKirchhoffStressPrivate(
            const StvkHenckyWithVonMisesModel2<T>::StrainData& trial_strain_data,
                                                Matrix3<T>* tau) const {
    // Calculate the singular values of Kirchhoff stress
    // Σᵢ = 2μ log(σᵢ) + λ ∑ᵢ log(σᵢ)
    Vector3<T> sigma_tau;
...
void StvkHenckyWithVonMisesModel2<T>::ApplyReturnMapping(
      ...
      T f_tau = EvalYieldFunction(strain_stress_data_trial.deviatoric_tau);
      bool in_yield_surface =  f_tau <= 0.0;
      if (!in_yield_surface) {

        // project trial strain onto yield surface in the principal frame
        Vector3<T> nu = kSqrt3Over2
                            *strain_stress_data_trial.deviatoric_tau
                            /strain_stress_data_trial.deviatoric_tau.norm();
        T delta_gamma = f_tau/(3.0*this->mu_);
        // Update the singular values of Hencky strain ε
        Vector3<T> log_Sigma_new = strain_stress_data_trial.log_Sigma - delta_gamma*nu;
        ...
        // proj(Fᴱ)
        *F_trial = strain_stress_data_trial.U
...
void StvkHenckyWithVonMisesModel2<T>::
        ProjectDeformationGradientToYieldSurface(
            const StvkHenckyWithVonMisesModel2<T>::StressData& trial_stress_data,
            StvkHenckyWithVonMisesModel2<T>::StrainData* trial_strain_data,
    ...
    if (!in_yield_surface) {
        ...
        Vector3<T> nu = StvkHenckyWithVonMisesModel2::sqrt_32
