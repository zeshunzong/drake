...
class StvkHenckyWithVonMisesModel2 : public ElastoPlasticModel<T> {
 public:
  ...
  explicit StvkHenckyWithVonMisesModel2(const T& yield_stress);
 ...
 private:
   ...
   * U, V from svd such that F = U Σ Vᵀ
   * Sigma: diag(Σ), vec3
   * OneOverSigma: 1./diag(Σ), vec3
   * log_Sigma: log(diag(Σ)), vec3
   * log_Sigma_trace: sum(log(diag(Σ))), scalar
   * deviatoric_tau: the deviatoric component of τ in the principal frame, vec3
  ...
  T b01(PsiSigmaDerivative& psi_derivative, int i, int j) const {
    return (i == j ? psi_derivative.m01 + psi_derivative.p01
                   : psi_derivative.m01 - psi_derivative.p01) *
           0.5;
  }

  T b12(PsiSigmaDerivative& psi_derivative, int i, int j) const {
    return (i == j ? psi_derivative.m12 + psi_derivative.p12
                   : psi_derivative.m12 - psi_derivative.p12) *
           0.5;
  }

  T b20(PsiSigmaDerivative& psi_derivative, int i, int j) const {
    return (i == j ? psi_derivative.m02 + psi_derivative.p02
                   : psi_derivative.m02 - psi_derivative.p02) *
           0.5;
  }
  ...
  /**
   * Returns the yield function f evaluated at deviatoric_tau.
   * f(dev(τ)) = sqrt(3/2)‖ dev(τ) ‖ - τ_c, where dev(τ) is the deviatoric
   * component of Kirchhoff stress in the principal frame
   */
  T EvalYieldFunction(const Vector3<T>& deviatoric_tau) const;
  ...
  void ProjectDeformationGradientToYieldSurface(
      const StressData& trial_stress_data, StrainData* trial_strain_data,
  ...
  void CalcKirchhoffStressPrivate(const StrainData& trial_strain_data,
                                  Matrix3<T>* tau) const;

  // sqrt(3.0/2.0)
  T sqrt_32 = 1.224744871391589;
  const T kSqrt3Over2 = 1.224744871391589;
  T yield_stress_;
  T epsilon_threshold_ =
      1e-12;  // prevent singular values from being too close to zero
};            // class StvkHenckyWithVonMisesModel2
