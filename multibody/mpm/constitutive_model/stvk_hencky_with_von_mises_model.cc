  ...
  for (int ij = 0; ij < 9; ++ij) {
    ...
    for (int rs = 0; rs <= ij; ++rs) {
      ...
      (*dPdF)(ij, rs) = (*dPdF)(rs, ij) =
         ...
         + B02(psi_derivatives, 1, 1) * ssd.U(i, 0) * ssd.V(j, 2) * ssd.U(r, 0) * ssd.V(s, 2) // 0202, B02(1,1)=M_0202
         
         + B02(psi_derivatives, 1, 0) * ssd.U(i, 0) * ssd.V(j, 2) * ssd.U(r, 2) * ssd.V(s, 0) // 0220, B02(1,0)=M_0220
         
         + B02(psi_derivatives, 0, 1) * ssd.U(i, 2) * ssd.V(j, 0) * ssd.U(r, 0) * ssd.V(s, 2) // 2002, B02(0,1)=M_2002=M_0220
         
         + B02(psi_derivatives, 0, 0) * ssd.U(i, 2) * ssd.V(j, 0) * ssd.U(r, 2) * ssd.V(s, 0);// 2020, B02(0,0)=M_2020=M_0202
      // clang-format off
    }
  }
}
...
void StvkHenckyWithVonMisesModel<T>::ApplyReturnMapping(
  ...
  if (!in_yield_surface) {
    ...
    const Vector3<T> nu = kSqrt3Over2 * strain_stress_data_trial.deviatoric_tau.normalized();
